...
- Each issue's `<head>` carries `og:title`/`og:description`/`og:image`/`og:url` for social-share previews. The image is rendered from a per-issue `og-card.html` via headless Chromium (README.md has the exact command) — not hand-drawn, since OG tags need a real image file.
